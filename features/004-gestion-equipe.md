# Feature 004 — Gestion de l'équipe

- **Statut** : Fait
- **Dépend de** : `002` (store persisté, module `personnes`, `schema.js`, plugin de persistance débouncé). S'appuie aussi sur `003` (composant `IndicateurSauvegarde`, module `src/domain/libelles.js`, action `cabinet/majParametres` non requise mais mêmes patterns).
...
C'est le **deuxième écran réel** de l'application et le **premier vrai CRUD de liste**. À l'issue de `002`, la collection `personnes` existe dans l'état (vide, persistée automatiquement) mais **aucun écran ne permet de la consulter ni de la modifier** : `EquipeView.vue` est un simple placeholder.

La feature `004` rend l'écran **Équipe** opérationnel pour le référent (public **peu à l'aise avec l'informatique**) : **lister, ajouter, modifier, archiver et restaurer** les personnes du cabinet. Chaque personne porte les informations qui serviront de socle aux features suivantes : identité (prénom, nom), **statut** (titulaire / remplaçant), **couleur** de repère pour le planning, **quotité** (temps de travail), **dates** d'arrivée/départ, et un **indicateur d'activité** (`actif`) qui matérialise le **soft-delete**.

Chaque enregistrement valide est **persisté automatiquement** (plugin de persistance débouncé de `002`), avec un **retour visuel clair** réutilisant `IndicateurSauvegarde` (`003`). Les personnes archivées ne sont **jamais supprimées physiquement** (règle d'intégrité [02 §Intégrité](../docs/architecture/02-modele-de-domaine.md)) : elles restent référençables par les plannings historiques (features `010`+).

**Hors périmètre `004`** (à ne pas implémenter ici) :

- **Souhaits & préférences** d'une personne (`preferences[]`) — c'est la feature `005` (route `/equipe/:id/souhaits`). La feature `004` initialise `preferences` à `[]` et ne l'édite pas.
...
Entité **`Personne`**, déjà décrite dans le modèle de domaine ([02 §Personne](../docs/architecture/02-modele-de-domaine.md)) et présente en tant que collection racine `personnes.items` (vide) depuis `002`. **Aucune nouvelle structure, aucun nouveau champ, aucune migration.**

Champs de `Personne` **manipulés** par `004` :

| champ | type | oblig. | rôle `004` |
|---|---|---|---|
...
| `preferences` | Preference[] | oui | initialisé à `[]`, **non édité** (feature `005`) |
| `createdAt` / `updatedAt` | ISO UTC | oui | posés/rafraîchis automatiquement (`new Date().toISOString()`) |

> **Convention de soft-delete confirmée** dans `schema.js` et [02](../docs/architecture/02-modele-de-domaine.md) : c'est le booléen **`actif`** (`false` = archivée), **pas** un `archivedAt`. (Le champ `archivee` existe, lui, pour `Tournee`, feature `006`.)
...
Module `personnes` ([04-gestion-etat-vuex.md](../docs/architecture/04-gestion-etat-vuex.md), [instructions/etat-vuex.md](../docs/instructions/etat-vuex.md)). Après `002` il expose : `state {items: []}`, getters `byId` et `actifs`, mutation `REPLACE`. Il **n'a pas encore d'action CRUD** (elles étaient explicitement différées à `004`).
...
Les actions **orchestrent** ; la construction/normalisation d'une personne vit dans le **domaine** (§5), jamais dans le store (règle d'or #10). Les horodatages techniques (`updatedAt`) sont posés dans l'action via `new Date().toISOString()` (même convention que `cabinet/majParametres` en `003`).
...
L'écran **lit** (sans jamais les muter) l'état racine de sauvegarde posé en `002` pour le retour visuel, exactement comme `003` :
...
- **`creerPersonne(champs)`** → `Personne` : construit une personne **complète et normalisée** à partir d'un objet partiel (les champs saisis dans le formulaire), en appliquant les **valeurs par défaut** et en générant les champs techniques. Fonction **pure** (hors `genId()` et `new Date().toISOString()`, tolérés car techniques — même usage que `schema.js`).
  ...
  - `preferences` : `champs.preferences ?? []` (jamais éditées en `004`).
...
Demande de confirmation générique, bâtie **au-dessus de `ModaleBase`** (réutilisable pour l'archivage ici, et plus tard réinitialisation/import `008`, suppression tournée `006`…).
...
- **Indicateur de sauvegarde** : `IndicateurSauvegarde` (`003`) alimenté par `statutSauvegarde`/`derniereSauvegarde` (root state via `mapState`) et `apres-edition="aEdite"` (passe à `true` après le 1er ajout/édition/archivage/restauration — même logique que `ParametresView`). Reprend aussi l'encart `ERREUR_CHARGEMENT` de `003`.
...
- **Tri d'affichage** (computed) : actives et archivées triées par `nom` puis `prenom` via `localeCompare('fr')` (présentation ; `ordreAffichage` non utilisé en `004`).
...
@import 'bootstrap/scss/modal';   // composant modale (feature 004)
```
...
- La directive `v-debounce` (enregistrée globalement en `003`) **n'est pas nécessaire** ici (formulaire à validation explicite, pas de saisie auto-persistée en continu).
...
> Contrairement à `003` (auto-save par champ d'un singleton), un CRUD de liste en modale utilise une **validation + enregistrement explicite** (bouton « Enregistrer ») : c'est le modèle attendu par l'utilisateur pour créer/éditer une fiche.
...
- **Cohérence** : mêmes patterns que `003` (indicateur de sauvegarde, présentation des erreurs) ; « Ajouter » se comportera pareil sur les futurs écrans (`006`/`007`).
...
7. **Tri & `ordreAffichage`** — Tri **alphabétique** (nom puis prénom) en `004` ; `ordreAffichage` initialisé mais **non édité** (réordonnancement par glisser-déposer différé). **À confirmer**.

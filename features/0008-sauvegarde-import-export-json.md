# Feature 0008 — Sauvegarde import/export JSON

- **Statut** : À faire
- **Dépend de** : `0002` (couche de persistance + store : le **backend** de cette feature — actions `exporter` / `importer` / `reinitialiser`, mutation `REPLACE_ALL`, `toSaveDocument`/`fromSaveDocument`/`verifierIntegrite`, `migrate` — est **déjà implémenté et fonctionnel**), `0003` (écran **Paramètres du cabinet** : `ParametresView.vue`, ses sections `.parametres-section` et `IndicateurSauvegarde` déjà en place ; le plan `0003` §16 a **explicitement différé « Export / import / réinitialisation » à la `0008`, sur le même écran, en bloc séparé**). S'appuie sur les briques `DialogueConfirmation` / `IndicateurSauvegarde` (existant `0004`), `dateUtil` et `src/domain/libelles.js`.
...
Une seule route, déjà créée en `0003` et confirmée par [07-navigation-et-ecrans](../docs/architecture/07-navigation-et-ecrans.md) (`name: 'parametres'`, path `/parametres`) :

| Route | Écran | Changement `0008` |
|---|---|---|
...
> Aucune route n'est ajoutée ni modifiée (`router` inchangé). Le choix de placer le bloc dans **Paramètres** (plutôt qu'un écran dédié) applique le renvoi explicite du plan `0003` §16 : c'est un réglage/outil transverse du cabinet, peu fréquent, sa place est aux côtés des autres réglages.
...
Tout le socle vit **déjà** dans `src/store/index.js` (`0002`) et est **réutilisé tel quel** ([04-gestion-etat-vuex](../docs/architecture/04-gestion-etat-vuex.md)). Rappel des contrats consommés par l'UI :
...
**Aucune règle Vuelidate.** La seule entrée est un fichier ; sa validité (lisibilité, JSON valide, version de schéma supportée, intégrité référentielle) est **intégralement** vérifiée par l'action `importer` (`0002`) et **restituée** à l'utilisateur via `{ message, erreurs? }`. L'UI se contente de :
...
- **Cohérence** : mêmes patterns que `0003`/`0004`/`0006`/`0007` (confirmation destructive rouge, indicateur de sauvegarde, icône + texte) — l'utilisateur retrouve des gestes connus.
...
**Dépend de** : actions racines + module `ui` (`0002`), briques `DialogueConfirmation` (`0004`), `dateUtil` (`0002`).
...
1. **Emplacement UI : section « Sauvegarde » dans `ParametresView` via un composant dédié `BlocSauvegarde` (retenu)** — **Retenu** plutôt qu'un écran/route dédié : applique le renvoi explicite du plan `0003` §16 (« même écran, bloc séparé »), garde les réglages/outils du cabinet au même endroit, et évite d'ajouter une route pour un usage peu fréquent. **Composant dédié** (`components/parametres/BlocSauvegarde.vue`) plutôt qu'un gros bloc inline dans la vue : encapsule l'état local (fichier en attente, confirmations, résultat d'import), reste **testable/relisible** et n'alourdit pas `ParametresView`. **À confirmer.**
...
6. **`reinitialiser` : `clear()` + réécriture débouncée de l'état par défaut (comportement observé)** — `reinitialiser` fait `storageRepository.clear()` **et** `commit('REPLACE_ALL', etatParDefaut())` ; ce commit déclenche par ailleurs le plugin débouncé qui **réécrit** l'état par défaut ~400 ms plus tard. Résultat net : le storage contient l'état par défaut (équivalent à « vide »). **Bénin**, mais à connaître si l'on teste l'état de `localStorage` juste après l'effacement (il peut passer transitoirement par « effacé » puis « état par défaut sérialisé »). **Backend existant — hors périmètre `0008`** ; noté pour information, à remonter si un comportement « storage réellement absent » était souhaité.
